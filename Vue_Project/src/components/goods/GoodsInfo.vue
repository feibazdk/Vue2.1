            ...
            enter(el, done) {
                ...
                /*  小球动画优化思路：
                    1. 先分析导致 动画 不准确的 本质原因： 
                        a、我们把小球最终位移到的 位置，已经局限在了某一分辨率下的 滚动条未滚动的情况下；
                        b、只要分辨率和 测试的时候不一样，或者 滚动条有一定的滚动距离之后， 问题就出现了；
                    3. 经过分析，得到结论：不能把 位置的 横纵坐标 直接写死了，而是应该 根据不同情况，动态计算这个坐标值；
                    4. 经过分析，得出解题思路：
                        a、先得到 徽标的 横纵 坐标，再得到 小球的 横纵坐标，
                        b、然后 让 y 值 求差， x 值也求 差，得到的结果，就是横纵坐标要位移的距离
